...
async function resolveHealthIssue(query) {
  ...
  try {
    const payload = await fetch(`/api/icd11/search?q=${encodeURIComponent(text)}&limit=8`).then((response) => response.json());
...
async function renderIcdSearchResults(query, boxSelector, onSelect, limit = 12) {
  ...
  const payload = await fetch(`/api/icd11/search?q=${encodeURIComponent(query)}&limit=${limit}`).then((response) => response.json());
  clearNode(box);
...
async function init() {
  const knowledge = await fetch("/api/knowledge").then((response) => response.json());
